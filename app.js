...
const updatePage = async () => {
  ...
  const fruits = fruitsArray.map((item) => {
    const newFruitElt = document.createElement('div');
    newFruitElt.classList.add("fruit")
    newFruitElt.innerHTML = item.name
    newFruitElt.innerHTML += ": "
    newFruitElt.innerHTML += item.family

    // Append the new element to an existing part of the webpage
    const existingElement = document.getElementById('fruit-info');
    existingElement.append(newFruitElt);
  })
...
const nameElement = () => {
  ...
  const newNameElt = document.createElement('div');
  newNameElt.innerHTML = names;

  // Append the new element to an existing part of the webpage
  const existingElement = document.getElementById('cs1300-gallery');
  existingElement.append(newNameElt);
}
